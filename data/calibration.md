...
## On-device offline task-time re-scoring (mobile/src/lib/onnx)
- CLAUDE.md section 3.1 requires offline re-scoring to run entirely on-device via ONNX and be marked "approximate (offline)". The server's morning-sync score is computed from a much richer per-task feature snapshot (soil survey, shift roster, live site congestion, etc.) than the phone has any local source for once offline.
- `featureVector.ts` therefore falls back to documented neutral values for any of the 29 trained features it can't derive on-device: unrecognized/unknown categoricals use -1, the same "unmapped category" convention `ml/task_time/features.py`'s `encode_categoricals` already uses for a category it's never seen; unknown numerics default to a fleet-typical midpoint (e.g. visibility_m=800, temp_c=28) rather than 0, since 0 would silently imply an extreme condition; `ppe_compliance_flag` defaults to compliant (1) absent evidence otherwise, an optimistic rather than punitive default given CLAUDE.md section 2.1's operator-trust framing.
- These are assumptions, not measurements — every offline estimate lists which fields were approximated in its `reasons[]`, and the UI (TodayScreen) labels the whole figure "~Re-scored offline ... (approximate)" rather than presenting it as equivalent to the server's fully-informed, SHAP-explained P50/P90.
